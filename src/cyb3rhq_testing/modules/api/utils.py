...
from cyb3rhq_testing.constants.api import CYB3RHQ_API_PROTOCOL, CYB3RHQ_API_HOST, CYB3RHQ_API_PORT, CYB3RHQ_API_USER, \
                                        CYB3RHQ_API_PASSWORD, LOGIN_ROUTE, USERS_ROUTE, RESOURCE_ROUTE_MAP, \
                                        TARGET_ROUTE_MAP
from cyb3rhq_testing.constants.paths.api import CYB3RHQ_API_CERTIFICATE
from cyb3rhq_testing.utils.file import read_json_file
...
def get_base_url(protocol: str = CYB3RHQ_API_PROTOCOL, host: str = CYB3RHQ_API_HOST, port: str = CYB3RHQ_API_PORT) -> str:
    """Get Cyb3rhq API's base URL.
    ...
    Returns:
        base_url (str): Cyb3rhq API's base URL
    """
...
def set_authorization_header(user: str = None, password: str = None) -> dict:
    ...
    """
    user = CYB3RHQ_API_USER if user is None else user
    password = CYB3RHQ_API_PASSWORD if password is None else password
...
def login(user: str = CYB3RHQ_API_USER, password: str = CYB3RHQ_API_PASSWORD,
          timeout: int = session_parameters.default_timeout, login_attempts: int = 3, backoff_factor: float = 0.5,
          host: str = CYB3RHQ_API_HOST, port: str = CYB3RHQ_API_PORT, protocol: str = CYB3RHQ_API_PROTOCOL
    ...
    response = session.post(url, headers=set_authorization_header(user, password), verify=CYB3RHQ_API_CERTIFICATE,
                                timeout=timeout)
...
def allow_user_to_authenticate(user_id: str = None) -> requests.Response:
    ...
    response = requests.put(url + params, headers=authentication_headers, verify=CYB3RHQ_API_CERTIFICATE)
...
def manage_security_resources(method: str = 'get', resource: Union[dict, str] = None,
    ...
    authentication_headers = login()[0]
    response = getattr(requests, method)(url, headers=authentication_headers, verify=CYB3RHQ_API_CERTIFICATE, json=payload)
...
def relate_resources(test_metadata: dict) -> None:
    ...
    for origin in relationships:
        ...
        for idx, target_value in enumerate(target_values):
            ...
            response = requests.post(url, headers=login()[0], verify=CYB3RHQ_API_CERTIFICATE)
            if response.status_code != 200 or response.json()['error'] != 0:
...
def remove_resources_relationship(origin_resource: dict = None, target_resource: dict = None) -> dict:
    ...
    response = requests.delete(url, headers=login()[0], verify=CYB3RHQ_API_CERTIFICATE)
    if response.status_code != 200 or response.json()['error'] != 0:
...
def compare_config_api_response(configuration, section):
    ...
    Args:
        configuration (dict): Dictionary with Cyb3rhq manager configuration.
...
def get_manager_configuration(section=None, field=None):
    """Get Cyb3rhq manager configuration response from API using GET /manager/configuration

    References: https://documentation.cyb3rhq.com/current/user-manual/api/reference.html#operation/
                api.controllers.manager_controller.get_configuration

    Args:
        section (str): cyb3rhq configuration section, E.g: "active-response", "ruleset"...
        field   (str): section child. E.g, fields for ruleset section are: decoder_dir, rule_dir, etc

    Returns:
        `obj`(str or map): active configuration indicated by Cyb3rhq API. If section and field are selected, it will
    ...
    assert response.json()['error'] == 0, f"Cyb3rhq API response status different from 0: {response.json()}"
...
def get_api_details_dict(protocol=CYB3RHQ_API_PROTOCOL, host=CYB3RHQ_API_HOST, port=CYB3RHQ_API_PORT, user=CYB3RHQ_API_USER, password=CYB3RHQ_API_PASSWORD,
