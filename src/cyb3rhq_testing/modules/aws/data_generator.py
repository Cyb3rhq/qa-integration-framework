...
from cyb3rhq_testing.constants.aws import *
from cyb3rhq_testing.utils.random import get_random_ip, get_random_port, get_random_string
        ...
        return json.dumps(
            {
                ...
                'detail': {
                    ...
                    'actor': 'resources.cyb3rhq.com',
                    'summary': {
                        'Description': 'S3 Bucket uses IAM policy to grant read rights to Everyone.',
                        'Bucket': {
                            'resources.cyb3rhq.com': 1
                        },
                        'Record Count': 1,
                        'ACL': {
                            'resources.cyb3rhq.com': [
                                {
                                    'Owner': {
                                        'DisplayName': 'cyb3rhq',
...
class ServerAccessDataGenerator(DataGenerator):
    ...
    def get_data_sample(self):
        ...
        for _ in range(5):
            data.append(
                [
                    str(uuid4()), 'cyb3rhq-server-access-integration-tests',
                    self.date.strftime('[%d/%b/%Y:%H:%M:%S %z]'), get_random_ip(),
                    f"arn:aws:iam::{RANDOM_ACCOUNT_ID}:user/fake.user", get_random_string(16).upper(),
                    'REST.GET.WEBSITE', '-', 'GET, /cyb3rhq-server-access-integration-tests?website= HTTP/1.1',
