...
from cyb3rhq_testing.constants.daemons import CYB3RHQ_DB_DAEMON
from cyb3rhq_testing.constants.paths.sockets import QUEUE_DB_PATH, CYB3RHQ_DB_SOCKET_PATH
from cyb3rhq_testing.tools.db_administrator import DatabaseAdministrator
from cyb3rhq_testing.utils import services, secure_message


def delete_dbs():
    """Delete all cyb3rhq-db databases."""
...
def query_wdb(command, response_only=True, multiple_responses=False) -> List[str]:
    """Make queries to cyb3rhq-db using the wdb socket.

    Args:
        command (str): cyb3rhq-db command alias. For example `global get-agent-info 000`.

    Returns:
        list: Query response data.
    """
    # If the wdb socket is not yet up, then wait or restart cyb3rhq-db
    if not os.path.exists(CYB3RHQ_DB_SOCKET_PATH):
        max_retries = 6
        for _ in range(2):
            retry = 0
            # Wait if the wdb socket is not still alive (due to cyb3rhq-db restarts). Max 3 seconds
            while not os.path.exists(CYB3RHQ_DB_SOCKET_PATH) and retry < max_retries:
                time.sleep(0.5)
                retry += 1

            # Restart cyb3rhq-db in case of wdb socket is not yet up.
            if not os.path.exists(CYB3RHQ_DB_SOCKET_PATH):
                services.control_service('restart', daemon=CYB3RHQ_DB_DAEMON)

        # Raise custom exception if the socket is not up in the expected time, even restarting cyb3rhq-db
        if not os.path.exists(CYB3RHQ_DB_SOCKET_PATH):
            raise Exception('The wdb socket is not up. cyb3rhq-db was restarted but the socket was not found')

    # Create and open the socket connection with cyb3rhq-db socket
    sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    sock.connect(CYB3RHQ_DB_SOCKET_PATH)
    data = []
...
def make_sqlite_query(db_path: str, query_list: List[str]) -> None:
    ...
    """
    services.control_service('stop', daemon=CYB3RHQ_DB_DAEMON)
    ...
    try:
        ...
        services.control_service('start', daemon=CYB3RHQ_DB_DAEMON)
