...
from cyb3rhq_testing.constants.daemons import CLUSTER_DAEMON, API_DAEMON, CYB3RHQ_AGENT, CYB3RHQ_MANAGER, CYB3RHQ_AGENT_WIN
from cyb3rhq_testing.constants.paths.binaries import BIN_PATH, CYB3RHQ_CONTROL_PATH
from cyb3rhq_testing.constants.paths.sockets import CYB3RHQ_SOCKETS, CYB3RHQ_OPTIONAL_SOCKETS
from cyb3rhq_testing.constants.paths.variables import VAR_RUN_PATH, VERSION_FILE
from cyb3rhq_testing.constants.platforms import MACOS, SOLARIS, WINDOWS
...
def get_service() -> str:
    """
    Retrieves the name of the Cyb3rhq service running on the current platform.

    Returns:
        str: The name of the Cyb3rhq service. It can be either 'cyb3rhq-manager'
             or 'cyb3rhq-agent'.
    ...
    if platform.system() in ['Windows', WINDOWS]:
        return CYB3RHQ_AGENT

    else:  # Linux, sunos5, darwin, aix...
        service = subprocess.check_output([
            CYB3RHQ_CONTROL_PATH, "info", "-t"
        ], stderr=subprocess.PIPE).decode('utf-8').strip()

    return CYB3RHQ_MANAGER if service == 'server' else CYB3RHQ_AGENT


def get_version() -> str:
    """
    Retrieves the version of the Cyb3rhq software installed on the current platform.

    Returns:
        str: The version of the Cyb3rhq software.
...
def control_service(action, daemon=None, debug_mode=False):
    ...
    Args:
        ...
        daemon (str, optional): Name of the daemon to be controlled. None for the whole Cyb3rhq service. Default `None`.
    ...
    if sys.platform == WINDOWS:
        if action == 'restart':
            ...
            for _ in range(error_windows_retry):
                command = subprocess.run(["net", action, "Cyb3rhqSvc"], stderr=subprocess.PIPE)
                ...
                if result == 0:
                    ...
                    if 'The service is starting or stopping' in error:
                        time.sleep(1)
                        continue
                    if action == 'stop' and 'The Cyb3rhq service is not started.' in error:
                        result = 0
                        break
        ...
        if daemon is None:
            if sys.platform == MACOS or sys.platform == SOLARIS:
                result = subprocess.run(
                    [CYB3RHQ_CONTROL_PATH, action]).returncode
            else:
                result = subprocess.run(
            ...
            if action == 'restart':
                ...
                sockets.delete_sockets(CYB3RHQ_SOCKETS[daemon])
            else:
...
def check_all_daemon_status():
    ...
    if sys.platform == WINDOWS:
        current_daemon = 'CYB3RHQ_AGENT_WIN'
        daemons_status[current_daemon] = check_if_process_is_running(CYB3RHQ_AGENT_WIN)
    else:
        control_status_output = subprocess.run([CYB3RHQ_CONTROL_PATH, 'status'],
...
def wait_expected_daemon_status(target_daemon=None, running_condition=True, timeout=10, extra_sockets=[]):
    """Wait until Cyb3rhq daemon's status matches the expected one. If timeout is reached and the status didn't match,
       it raises a TimeoutError.

    Args:
        target_daemon (str, optional):  Cyb3rhq daemon to check. Default `None`. None means all.
    ...
    while elapsed_time < timeout and not condition_met:
        if sys.platform == WINDOWS:
            condition_met = check_if_process_is_running(CYB3RHQ_AGENT_WIN) == running_condition
        else:
            control_status_output = subprocess.run([CYB3RHQ_CONTROL_PATH, 'status'],
                                                   stdout=subprocess.PIPE).stdout.decode()
            condition_met = True
            for lines in control_status_output.splitlines():
                ...
                if current_daemon == target_daemon or target_daemon is None:
                    if current_daemon in CYB3RHQ_SOCKETS.keys():
                        socket_set = {path for path in CYB3RHQ_SOCKETS[current_daemon]}
                    else:
                        socket_set = set()
                    # We remove optional sockets and add extra sockets to the set to check
                    socket_set.difference_update(CYB3RHQ_OPTIONAL_SOCKETS)
