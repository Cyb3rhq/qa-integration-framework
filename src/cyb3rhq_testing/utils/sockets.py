...
from cyb3rhq_testing.tools.socket_controller import SocketController
from cyb3rhq_testing.constants.paths.sockets import QUEUE_SOCKETS_PATH, CYB3RHQ_DB_SOCKET_PATH, \
                                                  MODULESD_C_INTERNAL_SOCKET_PATH, \
                                                  ACTIVE_RESPONSE_SOCKET_PATH
from cyb3rhq_testing.utils.network import UDP


def delete_sockets(path=None):
    """Delete a list of Cyb3rhq socket files or all of them if None is specified.
    ...
    try:
        if path is None:
            ...
            if os.path.exists(CYB3RHQ_DB_SOCKET_PATH):
                os.remove(CYB3RHQ_DB_SOCKET_PATH)
...
def send_request_socket(query, socket_path=CYB3RHQ_DB_SOCKET_PATH):
...
def send_message_to_syslog_socket(message, port, protocol, manager_address="127.0.0.1"):
    """Send a message to the syslog server of cyb3rhq-remoted.
