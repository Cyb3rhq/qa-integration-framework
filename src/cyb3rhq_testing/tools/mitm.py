...
from cyb3rhq_testing.constants.users import CYB3RHQ_UNIX_USER, CYB3RHQ_UNIX_GROUP
from cyb3rhq_testing.utils import secure_message
from cyb3rhq_testing.utils.cluster import CLUSTER_DATA_HEADER_SIZE, cluster_msg_build
...
class StreamHandler(socketserver.BaseRequestHandler):
    ...
    def default_cyb3rhq_handler(self):
        """Default cyb3rhq daemons TCP handler method for MITM server."""
...
class DatagramHandler(socketserver.BaseRequestHandler):
    ...
    def default_cyb3rhq_handler(self):
        """Default cyb3rhq daemons UDP handler method for MITM server."""
        data = self.request[0]
...
class ManInTheMiddle:
    ...
    def run(self, *args):
        ...
        if isinstance(self.listener_socket_address, str):
            uid = pwd.getpwnam(CYB3RHQ_UNIX_USER).pw_uid
            gid = grp.getgrnam(CYB3RHQ_UNIX_GROUP).gr_gid
